...
│   ├── host/               #   x86 host-side test binaries
│   │   └── debug/
│   ├── target/              #   RISC-V cross-compiled binaries
│   │   ├── release/         #     production canny_rv.elf
│   │   └── debug/           #     per-test ELF binaries
│   └── bench/                #   gem5 benchmark ELF binaries (scalar + RVV)
├── docs/                   # Generated Doxygen HTML output
├── external/                # Third-party dependencies (GoogleTest, gem5 helpers)
...
│   ├── assets/               #   test-only fixture images
│   ├── unit_tests.cpp         #   host-side GoogleTest suite
│   ├── qemu_tests.cpp          #   target-side QEMU equivalence suite
│   └── benchmark.cpp           #   gem5 benchmarking harness
├── tools/                   # gem5 launch script and profiling utilities
│   └── run_gem5.py
├── m5out/                   # gem5 statistics output (generated, gitignored)
├── compile_commands.json    # Generated by build system, used by clangd/IDE tooling
...
| :--- | :--- | :--- |
| `make all` | Full Build | Compiles the main pipeline (RISC-V) to `build/target/release/canny_rv.elf`. |
| `make run` | QEMU Run | Executes the production ELF under QEMU for a quick functional check. |
| `make tests` | Host Test | Compiles and runs GoogleTest natively (x86) for logic verification. |
| `make qemu-tests` | Target Test | Compiles and runs the RVV equivalence suite under QEMU at VLEN 128/256/512. |
| `make gem5-run NAME=x` | Unit Simulate | Builds `tests/x.cpp` and runs it on gem5 (e.g., `make gem5-run NAME=sobel`). |
| `make bench-rvv` / `make bench-scalar` / `make bench-all` | Benchmark | Runs the gem5 benchmark harness across optimization levels. |
| `make list-tests` | Discovery | Scans the `tests/` folder and lists all testable `.cpp` files. |
| `make size-report` | Binary Size | Prints the section sizes of the production ELF. |
| `make autovec-report` | Vectorization | Counts emitted vector instructions in the production ELF. |
| `make clean` | Cleanup | Wipes `build/`, `m5out/`, and `results/`. |

**Configuration Overrides** — hardware parameters can be overridden from the command line:

```bash
make gem5-run NAME=sobel        # Run a single test stage on gem5
make bench-rvv-O3               # Run the RVV benchmark at a specific optimization level
```
