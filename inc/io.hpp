                                ...
                                metadata_t<PixelT>& metadata)
{
    ...
    if (!pixel_count) return Status::E_NOK;
    ...
    // Construct the file path based on the build configuration
#if defined(TESTS)
    std::filesystem::path file_path = std::filesystem::path("./tests/assets/") / file_name;
#else
    std::filesystem::path file_path = std::filesystem::path("./assets/") / file_name;
#endif
    std::ifstream file(file_path, std::ios::binary);

    if (!file || !file.read(reinterpret_cast<char*>(metadata.buffer.get()), total_bytes))
        return file ? Status::E_NOK : Status::E_INVAL_DIR;
...
{
    if (!metadata.buffer || !metadata.pixel_count)
        return metadata.buffer ? Status::E_NOK : Status::E_INVAL_PTR;
    ...
    if (!file) return Status::E_INVAL_DIR;
