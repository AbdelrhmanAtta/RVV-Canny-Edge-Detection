...
int main()
{
    ...
    stat = image::io::load_raw<uint8_t>("tiger.raw", img_spatial);
    if (Status::E_OK != stat) return static_cast<int>(stat);

    stat = image::io::load_raw<uint8_t>("tiger.raw", img_separable);
    if (Status::E_OK != stat) return static_cast<int>(stat);
    ...
    if (Status::E_OK == stat) {
        ...
        stat = image::io::save_raw<uint8_t>("tiger_spatial.raw", img_spatial);
        if (Status::E_OK != stat) return static_cast<int>(stat);
    } else {
    ...
    if (Status::E_OK == stat) {
        ...
        stat = image::io::save_raw<uint8_t>("tiger_separable.raw", img_separable);
        if (Status::E_OK != stat) return static_cast<int>(stat);
    } else {
        return static_cast<int>(stat);
    }
