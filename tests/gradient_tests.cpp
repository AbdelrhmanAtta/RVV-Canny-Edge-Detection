...
int main()
{
    ...
    out_l1.width = w;
    out_l1.height = h;
    out_l1.pixel_count = pixel_count;
    out_l1.aligned_buffer_size = aligned_size;
    out_l1.buffer.reset(static_cast<uint8_t*>(utils::memory::aligned_alloc(64, aligned_size)));

    image::io::metadata_t<uint8_t> out_l2;
    out_l2.width = w;
    out_l2.height = h;
    out_l2.pixel_count = pixel_count;
    out_l2.aligned_buffer_size = aligned_size;
    ...
    auto d_l1 = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1);
    auto d_l2 = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3);

    std::cout << "L1 Magnitude Time: " << d_l1.count() << " us" << std::endl;
    std::cout << "L2 Magnitude Time: " << d_l2.count() << " us" << std::endl;
